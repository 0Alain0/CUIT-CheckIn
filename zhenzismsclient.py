...
class ZhenziSmsClient(object):
	...
	def send(self, params):
		...
		response = requests.post(
			self.apiUrl+'/sms/v2/send.do', data=data, verify=False)
	...
	def balance(self):
		...
		requests.packages.urllib3.disable_warnings(InsecureRequestWarning)
		response = requests.post(
			self.apiUrl+'/account/balance.do', data=data, verify=False)
		result = str(response.content, 'utf-8')
		return result
	...
	def findSmsByMessageId(self, messageId):
		...
		requests.packages.urllib3.disable_warnings(InsecureRequestWarning)
		response = requests.post(
			self.apiUrl+'/smslog/findSmsByMessageId.do', data=data, verify=False)
		result = str(response.content, 'utf-8')
		return result
